...
## ⚡ Tối Ưu Hóa Hiệu Suất (Jetson Nano)
Hệ thống được thiết kế đặc biệt với kỹ thuật **Frame Skipping** và **State Caching**. Khối lượng tính toán nặng (nhận diện khuôn mặt dlib, trích xuất điểm mốc) chỉ được thi hành mỗi 3 khung hình 1 lần (tương đương ~10 FPS). Trong các khung hình bị bỏ qua, hệ thống sử dụng trạng thái đệm (cache) để tiếp tục duy trì dòng chảy ứng dụng ở mức 30 FPS mượt mà. 
Sự kết hợp này giúp tiết kiệm xấp xỉ **66% tải CPU**, mang lại trải nghiệm không giật lag trên các thiết bị nhúng yếu như **Nvidia Jetson Nano**.
